...
public class ShiroController {
    ...
    public String login(User user, BindingResult bindingResult, RedirectAttributes redirectAttributes){
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("message", "密 码不正确");
        ...
        //验证是否登录成功
        if(currentUser.isAuthenticated() && redirectAttributes.getFlashAttributes().size() > 0){
    ...
    public String getUserList(Map<String, Object> model){
        model.put("userList", jdbcTemplate.queryForList("select username,password from t_user"));
        return "users";
    }
